        ...
        <v-list-item three-line class="px-3" :key="project.gameId" :to="`/game/${project.gameId}`">
          <v-list-item-content class="py-0">
            <v-col cols="7" class="px-0">
              ...
              <v-list-item-subtitle class="mt-2">{{ project.genreName }}</v-list-item-subtitle>
              <v-list-item-subtitle > {{ project.leftDay }}일 남음 <span class="primary--text">{{ fundingProgress(project.aim, project.leftPrice) }}% 달성</span></v-list-item-subtitle>
            </v-col>
            <v-col cols="5">
